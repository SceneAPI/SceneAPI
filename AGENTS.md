...
`sceneapi/server/` contains the FastAPI service: `api/v1/` routes, `schemas/` wire models, `services/` tenant logic, `orchestrator/` scheduling, `workers/` task handlers, `storage/`, `db/`, and `adapters/` for backend registration (`sceneapi/*.py` are the public facades; the top-level `sfmapi/` package is a deprecated one-release alias shim over `sceneapi`, removed in 0.2.0). Tests use `tests/{unit,integration,e2e,contract,conformance}/`. SDKs live in the sibling `../sfmapi-sdk` repository; `scripts/regen_sdk.py` copies this repo's OpenAPI document there and regenerates client code. Docs are in `docs/`, deploy assets in `deploy/`, migrations in `alembic/`, benchmarks in `bench/`.

## Build, Test, and Development Commands

- `uv venv` then `uv sync --extra dev`: create the Python 3.12 environment.
- `uv run alembic upgrade head`: apply database migrations.
- `uv run uvicorn sceneapi.server.main:app --reload`: run the API locally.
...
- `uv run ruff check sceneapi sfm_hub sfmapi tests scripts` and `uv run ruff format --check sceneapi sfm_hub sfmapi tests scripts`: lint, import-sort, and format-check (same scope as CI).
...
Python targets 3.12 and uses the Ruff stack with a 100-character line length. Use 4-space indentation, explicit types where they clarify contracts, snake_case for functions/modules, PascalCase for classes, and UPPER_SNAKE_CASE for constants. Keep routes thin: schemas in `sceneapi/server/schemas/api/`, business logic in `sceneapi/server/services/`, and execution in `sceneapi/server/workers/tasks/`. Do not import concrete SfM engines; backends implement the protocols in `sceneapi.backends` and register via `sceneapi.runtime.register_backend` (container-service plugins build on `sceneapi.plugin_service`; the `sceneapi.server.*` tree is internal).
...
Start from `.env.example`; keep secrets out of git. Use `SCENEAPI_EPHEMERAL=true` for demos, and prefer explicit `SCENEAPI_DB_URL`, auth mode, queue backend, and storage settings in shared environments. Backend packages live outside this repo and register through adapters.
