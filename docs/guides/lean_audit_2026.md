...
- [x] 3.3 **Descriptor-registry base** for the `backend_config` /
...
- [x] 3.4 **`sfm_stage_service` diet**: table-driven recon-stage
...
- [x] 3.8 Split `sfm_hub/models.py` (70 KB) into modules; it is now
...
- [ ] 4.5 Absorb the in-process `api_launcher.py`/`cli.py`
...
- [x] 5.3 **Wire-format single-sourcing**: move `_ergonomics.py` /
  ...
  instead of its private copies. (M) — partial (2026-07-17): the
  golden-bytes leg is done for the surviving points-v1 binary format —
  server-owned `tests/contract/fixtures/golden_points_v1.bin`
  generated by the canonical encoder and pinned byte-identical to it
  by `tests/contract/test_f_golden_points_parity.py`, which also
  asserts server-decoder ↔ generated-Python-SDK value parity; the
  generated TS SDK decodes the same file against the same hardcoded
  table in `sfmapi-sdk/typescript/test/golden_points.test.ts`
  (sfmapi-sdk 04f90ea). The C++ golden leg is deferred until the
  cpp/ WIP lands (cpp/test carries ~19 dirty files; recipe noted in
  CLAUDE.md). The `_ergonomics` move out of `_generated/` trees, an
  SSE golden fixture, and repointing C++ contract tests at server
  fixtures remain.
- [x] 5.4 **depth/normal formats decision**: they are in CLAUDE.md +
  ...
  — done for the supported generated SDKs (2026-07-17, recommended
  option): `parse_depth_map` / `parse_normal_map` +
  `parseDepthMap` / `parseNormalMap` and their media-type / magic /
  header constants deleted from `sfmapi_client_gen/_ergonomics.py`
  and `typescript/src/_generated/ergonomics.ts` (+ `client.ts`
  re-exports + SDK-local synthesized tests) — sfmapi-sdk 414e27f;
  CLAUDE.md updated. Reintroduction requires a server emitter + an
  SFMAPI-SPEC entry first. The deprecated hand-rolled
  `python/sfmapi_client` / `typescript/src` copies and the C++ SDK
  still carry the parsers; they ride until the in-flight WIP on
  those trees lands (remove there during the 0.1.0 hand-rolled
  cleanup).
...
- [x] 6.3 **Custom-verb normalization** (pre-1.0 window): rename the
  ...
  same resource. Regen SDKs after. (M) — done: 8 verbs renamed,
  pipelines/{recipe} deprecated=True (bench unaffected),
  relocalize/localize cross-referenced; regen in the 0.0.2 close-out
- [x] 6.4 Move working docs (`*_proposal.md`, `*_checklist.md`,
  audits incl. this one) out of the published Sphinx nav into an
  internal section or `docs/_internal/`; flip `nitpicky=True` once
  the stale `SfmBackend` xrefs are fixed. (M) — partial 2026-07-17:
  11 working docs (6 proposals, 4 checklists, aip_audit_2026) moved
  to `docs/_internal/` (whole dir in `exclude_patterns`); this file
  + decisions.md stay at `docs/guides/` but are excluded from the
  site; index.md toctrees/links + cli.md + register link paths
  updated; strict `-W` build green (was red: this file sat in no
  toctree). nitpicky flip measured and reverted — 682 warnings /
  127 distinct targets (stdlib refs blocked by
  `intersphinx_disabled_reftypes`, sqlalchemy/pydantic without
  inventories, ~60 undocumented `sfmapi.server.*` hints), not a
  small stale-xref set; see TODO in `docs/conf.py`.
