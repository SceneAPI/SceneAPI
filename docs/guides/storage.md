...
|---|---|---|
| **Blobs** | Content-addressed immutable bytes for uploaded images | `sceneapi.server.storage.blobs` |
| **ImageSource** | Logical reference to *where* bytes live (upload / local / S3) | `sceneapi.server.sources.*` |
...
```{eval-rst}
.. autoclass:: sceneapi.server.storage.blobs.BlobStore
   :members:
   :no-index:
```
...
```{eval-rst}
.. autoclass:: sceneapi.server.sources.upload.UploadSource
   :members:
   :no-index:

.. autoclass:: sceneapi.server.sources.local.LocalPathSource
   :members:
   :no-index:

.. autoclass:: sceneapi.server.sources.s3.S3Source
   :members:
   :no-index:
```
...
```{eval-rst}
.. autoclass:: sceneapi.server.storage.s3_cache.S3Cache
   :members:
   :no-index:
```
...
```{eval-rst}
.. autoclass:: sceneapi.server.storage.snapshots.SnapshotStore
   :members:
   :no-index:
```
...
```{eval-rst}
.. automodule:: sceneapi.server.storage.mapping_input
   :members:
   :no-index:
```
...
```{eval-rst}
.. autofunction:: sceneapi.server.storage.workspace.gc_completed_jobs
   :no-index:
```
