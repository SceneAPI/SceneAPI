...
- [ ] **Wire cooperative cancellation.** `POST /v1/jobs/{id}:cancel` flips
...
- [ ] **Add the janitor.** `settings.janitor_interval_seconds` is defined but
...
- [ ] **Echo the resolved `provider`.** Core SfM stages run
...
- [ ] **Run provider resolution on utility stages.** `apply_provider_resolution`
...
- [ ] **Add `extra="forbid"` to inline request models.** `LocalizationRequest`,
...
- [ ] **Make `doctor` actually probe.** `sfm_hub/doctor.py::doctor_manifest`
...
- [ ] **Fix the error `type` URI in the spec.** SFMAPI-SPEC.md:351 uses
...
- [ ] **Re-run `scripts/regen_sdk.py` and commit the SDK repo.** The generated
...
- [ ] **Portable job-submission routes for the 6 Tier-3 capabilities.** The L36
...
- [ ] **Decide the fate of the 6 orphaned task handlers.** `ba`, `triangulate`,
...
- [ ] **Consume `pose_priors` in global/hierarchical mapping** — or scope the
...
- [ ] **Validate bundled-manifest capability strings against `ALL_KNOWN`.**
...
- [ ] **Validate manifest field formats.** `provider_id`, `entry_points`
...
- [ ] **Validate routing-profile providers on write.** `upsert_profile` (and the
...
- [ ] **C++ SDK POD structs for the new request bodies.** `cpp/include/sfmapi/specs.hpp`
...
- [ ] **hloc retrieval-method selection.** hloc ships NetVLAD / DIR / OpenIBL /
...
- [ ] **COLMAP 3.13 `global_mapper` rename.** `COLMAP_BACKEND_CONFIGS` targets the
...
- [ ] **CLAUDE.md points-binary header size.** CLAUDE.md:84 and :116 say "32 B
      header"; the truth is 44 (`app/schemas/points_binary.py:29`, and
      SFMAPI-SPEC §7.1 is correct). → CLAUDE.md.
- [ ] **Add implemented routes to the spec tables.** Missing from §6.x / §7:
...
- [ ] **Resolve the §10 vs Appendix D dense/mesh contradiction.** §10 lists
...
- [ ] **Remove C++ SDK vaporware.** `cpp/include/sfmapi/client.hpp` has
...
- [ ] **`install_plugin` over an existing install silently overwrites.**
...
- [ ] **Decide on the unused manifest fields.** `conformance`, `compatibility`,
...
- [ ] **Decide on dead orchestrator code.** `fair_share.pick_next_task` /
...
- [ ] **`ArtifactImportRequest` forward-ref cleanup.** `app/schemas/api/artifacts.py`
...
- [ ] **e2e tests for the new portable stages** (depends on Tier-2 routes).
- [ ] **Real-engine CI lane.** Most plugin tests mock the engine or skip when no
