...
- ``scripts/regen_sdk.py`` is the single entrypoint: dumps the
  ...
  - ``../sfmapi-sdk/python/scenesdk/`` — attrs-based dataclasses
    plus per-endpoint API methods (dist ``scenesdk``).
...
- Both generated SDKs ship with a small repo-owned ergonomics shim
  ...
  - Python: ``../sfmapi-sdk/python/scenesdk/_ergonomics.py`` —
  ...
  - TypeScript: ``../sfmapi-sdk/typescript/src/_generated/ergonomics.ts`` —
    ...
    ``@scenesdk/client/generated`` subpath.
...
- ``../sfmapi-sdk/python/sfmapi_client/`` (and its ``sfmapi-client``
  ...
  ``scenesdk._ergonomics``; consumers import the generated
  SDK (dist ``scenesdk``). The hand-rolled TypeScript layer in
...
- ``sceneapi/server/storage/blobs.py::get_blob_store()`` caches the in-memory
  ...
  When adding a new repo-owned file under ``scenesdk/``,
...
- The TypeScript SDK (``@scenesdk/client``) exposes the generated
  layer at ``@scenesdk/client/generated`` and the raw
  paths/components at ``@scenesdk/client/generated/openapi``.
