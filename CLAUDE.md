...
- Both generated SDKs ship with a small repo-owned ergonomics shim
  ...
  an SFMAPI-SPEC entry first. (The deprecated hand-rolled
  ``sfmapi_client`` / ``typescript/src`` copies and the C++ SDK still
  carry them until the in-flight SDK-repo WIP lands.)
