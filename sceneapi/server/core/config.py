"""Pydantic Settings — single source of truth for all sceneapi config."""
...
# Legacy env prefix honored for one release (removed in 0.2.0): any
# ``SFMAPI_X`` is read when the ``SCENEAPI_X`` form is absent, both from
# the process environment and from ``.env`` files. New-prefix values
# always win.
_LEGACY_ENV_PREFIX = "SFMAPI_"

_legacy_env_warning_emitted = False


def _warn_once_on_legacy_env(settings_cls: type[BaseSettings]) -> None:
    """Emit one DeprecationWarning per process when a legacy
    ``SFMAPI_<field>`` env var is about to take effect (its
    ``SCENEAPI_<field>`` counterpart is absent). Non-Settings names
    (e.g. ``SFMAPI_SDK_REPO``) never trigger it."""
    global _legacy_env_warning_emitted
    if _legacy_env_warning_emitted:
        return
    env_upper = {key.upper() for key in os.environ}
    active_legacy = sorted(
        _LEGACY_ENV_PREFIX + field.upper()
        for field in settings_cls.model_fields
        if _LEGACY_ENV_PREFIX + field.upper() in env_upper
        and "SCENEAPI_" + field.upper() not in env_upper
    )
    if not active_legacy:
        return
    _legacy_env_warning_emitted = True
    warnings.warn(
        f"SFMAPI_* environment variables are deprecated and will stop "
        f"working in sceneapi 0.2.0; rename to the SCENEAPI_* prefix "
        f"(found: {', '.join(active_legacy)}).",
        DeprecationWarning,
        stacklevel=4,
    )
...
class Settings(BaseSettings):
    ...
    def settings_customise_sources(
        ...
        # Construction-time legacy alias (SceneAPI migration Phase B):
        # the SFMAPI_-prefixed sources sit *after* their SCENEAPI_
        # counterparts, so a legacy var only fills fields the new prefix
        # left unset. Real env (either prefix) beats .env files.
        _warn_once_on_legacy_env(settings_cls)
        legacy_env = EnvSettingsSource(settings_cls, env_prefix=_LEGACY_ENV_PREFIX)
        legacy_dotenv = DotEnvSettingsSource(settings_cls, env_prefix=_LEGACY_ENV_PREFIX)
        return (
            init_settings,
            env_settings,
            legacy_env,
    ...
    # ``POST /v1/projects/{pid}/datasets:fromArchive`` decodes an
    ...
    # ``SCENEAPI_SPEC_URL`` (set explicitly to an empty string to omit
    ...
    # Auto-discover and register every installed ``sceneapi.backends``
    ...
    # `SCENEAPI_MCP_MODE=local` for new API-process mounts; the boolean is
