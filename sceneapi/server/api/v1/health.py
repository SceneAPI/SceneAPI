...
from sceneapi.server.schemas.api.common import (
    HealthResponse,
    ReadyzResponse,
    SpecResponse,
    SpecServerInfo,
...
async def readyz(session: AsyncSession = Depends(get_db)) -> JSONResponse:
    ...
    if getattr(settings, "redis_url", None) and settings.queue_backend != "inline":
        from sceneapi.server.orchestrator.queue import get_queue
...
async def version() -> VersionResponse:
    ...
    from sceneapi.server.schemas.api.common import BackendVersion
...
async def metrics() -> Response:
    ...
    from sceneapi.server.core import metrics as m
