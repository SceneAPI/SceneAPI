...
def _serve(args: argparse.Namespace) -> None:
    if args.mcp is not None:
        os.environ["SCENEAPI_MCP_MODE"] = args.mcp
...
def _mcp(args: argparse.Namespace) -> None:
    from sceneapi.server.mcp.server import main as mcp_main
...
def _check_backend(args: argparse.Namespace) -> None:
    ...
    if args.backend is not None:
        os.environ["SCENEAPI_BACKEND"] = args.backend

    from sceneapi.server.adapters.backend_contract import backend_contract_violations
...
def _scaffold_plugin(args: argparse.Namespace) -> None:
    from sceneapi.server.scaffolding import scaffold_plugin
...
def _scaffold_contract(args: argparse.Namespace) -> None:
    from sceneapi.server import scaffolding

    # sceneapi/server/core lives next to this CLI module; tests/unit is repo-relative.
    server_dir = Path(scaffolding.__file__).resolve().parent
    repo_root = server_dir.parents[1]  # <repo>/sceneapi/server -> <repo>
    ...
    print("Next steps:")
    print(
        f"  1. flesh out contract_dict() in sceneapi/server/core/{args.name}.py with the real standard"
    )
    print("  2. register it in sfmapi-cpp/tools/gen_contracts.py:")
    print(f"       from sceneapi.server.core import {args.name}")
    ...
    print(
        "  4. commit the sceneapi/server/core module, the test, and the generated "
...
def main(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(prog="sceneapi")
    ...
    mcp.add_argument(
        ...
        choices=("stdio", "http"),
        default=None,
        help="MCP transport. Defaults to SCENEAPI_MCP_MODE when it is stdio/http, otherwise stdio.",
    )
    ...
    check_backend.add_argument(
        ...
        action="append",
        default=[],
        help="Import a backend package/module before resolving SCENEAPI_BACKEND.",
    )
    check_backend.add_argument(
        "--backend",
        default=None,
        help="Backend registry name to check. Defaults to SCENEAPI_BACKEND.",
    )
    check_backend.add_argument(
        "--load-entry-points",
        action="store_true",
        help='Load installed [project.entry-points."sceneapi.backends"] before checking.',
    )
    ...
    scaffold_contract = subcommands.add_parser(
        "scaffold-contract",
        help=(
            "Scaffold an off-wire core contract: sceneapi/server/core/<name>.py "
    ...
    scaffold_contract.add_argument(
        "--core-dir",
        default=None,
        help="Override the sceneapi/server/core directory (defaults to the installed sceneapi/server/core).",
    )
    scaffold_contract.add_argument(
        "--tests-dir",
        default=None,
