...
from sceneapi.server.api.v1.artifacts import artifact_out
from sceneapi.server.core import artifacts as artifact_vocab
from sceneapi.server.core.capabilities import BackendInfo, empty_capabilities
...
from sceneapi.server.db.session import session_scope
from sceneapi.server.schemas.api.artifacts import (
    ArtifactConversionPlanRequest,
    ArtifactFormatOut,
    StageArtifactOut,
)
from sceneapi.server.schemas.api.backend_actions import BackendActionOut
from sceneapi.server.schemas.api.common import Page, to_out
from sceneapi.server.schemas.api.jobs import JobDetail, JobOut, JobStatus, TaskOut
from sceneapi.server.schemas.api.projects import ProjectOut
from sceneapi.server.schemas.api.reconstructions import (
    ReconstructionOut,
...
def resolve_tenant(tenant_id: str | None) -> str:
    ...
    if allowed is None:
        raise TenantViolationError(
            "SCENEAPI_MCP_TENANT_ID is required when SCENEAPI_AUTH_MODE is not 'none'"
        )
...
async def sfmapi_version() -> dict[str, Any]:
    ...
    from sceneapi.server.api.v1.health import version
...
# ``sceneapi/server/api/v1/{recon_stages,dataset_stages,sfm_stages,localize}.py``.
