...
configured backend with :func:`sceneapi.server.adapters.registry.get_backend` and
...
- Backend artifact I/O contracts live in ``sceneapi.server.adapters.backend_artifacts``
...
``progress: sceneapi.server.adapters.progress.ProgressReporter | None`` argument.
...
from sceneapi.server.adapters.progress import ProgressReporter
...
class Backend(BackendIdentity, Protocol):
    ...
    def capabilities(self) -> set[str]:
        ...
        :data:`sceneapi.server.core.capabilities.ALL_KNOWN`. Backend-native tools
