...
- a backend name, normally from ``SCENEAPI_BACKEND``;
...
Then set ``SCENEAPI_BACKEND=my_backend`` for process-wide default execution, or
...
def get_backend(name: str | None = None, *, provider: str | None = None) -> Backend:
    ...
    backend selector and overrides ``SCENEAPI_BACKEND`` when supplied.
    """
    ...
    if provider:
        ...
        if "@" in provider:
            raise KeyError(
                f"unknown sceneapi provider {provider!r}; registered providers: "
    ...
    # Legacy SFMAPI_BACKEND honored for one release (removed in 0.2.0).
    chosen = name or os.environ.get("SCENEAPI_BACKEND") or os.environ.get("SFMAPI_BACKEND")
    if not chosen:
        raise KeyError(
            "no sceneapi backend selected: set SCENEAPI_BACKEND or pass `name=` "
            f"explicitly. Registered backends: {list_backends()}; "
            f"registered providers: {list_backend_providers()}"
        )
    if chosen not in _REGISTRY:
        raise KeyError(
            f"unknown sceneapi backend {chosen!r}; registered: {list_backends()}. "
