...
``sceneapi/server/workers/dispatcher.py`` acquires a lease on every task it runs and
...
``sceneapi/server/main.py::lifespan`` (``_janitor_loop``); the DB predicates are kept as
...
from sceneapi.server.db.models import Job, JobEvent, Reconstruction, StageArtifact, Task
from sceneapi.server.orchestrator.lease import now_utc
from sceneapi.server.orchestrator.queue import get_shared_queue
from sceneapi.server.orchestrator.readiness import dependencies_ready, dependency_state
from sceneapi.server.services import job_service
...
async def run_janitor_once(session: AsyncSession) -> list[str]:
    ...
    with contextlib.suppress(Exception):
        from sceneapi.server.services.upload_service import gc_expired_uploads
