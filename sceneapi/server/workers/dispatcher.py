...
``sceneapi.server.workers.runner.run_task`` (which is ARQ's worker entrypoint
...
Backend wrappers stay thin:
  ...
  - ``sceneapi/server/workers/runner.py`` (ARQ):
...
from sceneapi.server.core.errors import BackendUnavailableError
...
from sceneapi.server.db.models import Job, Task
from sceneapi.server.db.session import get_session_factory
from sceneapi.server.orchestrator.lease import now_utc, refresh_lease, try_acquire_lease
from sceneapi.server.orchestrator.queue import get_shared_queue
...
from sceneapi.server.services import artifact_service, dataset_service, job_service
from sceneapi.server.workers.progress import (
    WorkerProgressReporter,
    reset_progress_reporter,
    set_progress_reporter,
)
from sceneapi.server.workers.tasks._registry import (
    get_registered,
    get_status_hook,
    get_success_hook,
)
...
WORKER_ID: str = (
    os.environ.get("SCENEAPI_WORKER_ID") or os.environ.get("SFMAPI_WORKER_ID") or new_id()
)
...
def _import_task_modules() -> None:
    """Import every public submodule under ``sceneapi.server.workers.tasks`` once.
    ...
    from sceneapi.server.workers import tasks

    for mod_info in pkgutil.iter_modules(tasks.__path__):
        if mod_info.name.startswith("_"):
            continue
        importlib.import_module(f"sceneapi.server.workers.tasks.{mod_info.name}")
...
# ``sceneapi.server.orchestrator.readiness`` (shared with the scheduler and the
