...
The scaffolded plugin uses the canonical :class:`sceneapi.backends.Plugin`
...
class ${class_name}Backend:
    ...
    def capabilities(self) -> list[str]:
        ...
        # The canonical vocabulary lives in sceneapi.server.core.capabilities.ALL_KNOWN.
        return []
...
def test_entry_point_registered() -> None:
    names = {ep.name for ep in m.entry_points(group="sceneapi.backends")}
...
# standard with cross-tier parity, e.g. sceneapi.contracts.colmap_db —
...
# A contract name becomes a Python module name (sceneapi/server/core/<name>.py), a
...
_CONTRACT_TEST_TEMPLATE = string.Template(
    '''\
"""Contract test for sceneapi.server.core.${name} (off-wire core contract)."""

from __future__ import annotations

import json

from sceneapi.server.core import ${name} as contract
