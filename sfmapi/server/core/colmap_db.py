...
imports it from ``sfmapi.server.core``), so it now lives in the public ``sfmapi``
package. This module re-exports the full surface so existing
``sfmapi.server.core.colmap_db`` imports keep working for one release; new code
...
warnings.warn(
    "sfmapi.server.core.colmap_db has moved to sfmapi.contracts.colmap_db; "
