...
the Format axis lives in :mod:`sfmapi.server.core.artifacts`. Keeping them separate
...
Repo-owned core contract / data standard (no plugin import). Operations
(:mod:`sfmapi.server.core.operations`) declare their ``consumes``/``produces`` over
these ids; pipelines (:mod:`sfmapi.server.core.pipelines`) type-check by threading them.
"""
