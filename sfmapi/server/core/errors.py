...
class SfmApiError(Exception):
    ...
    ``sfmapi/server/main.py::sfmapi_error_handler`` turns these into
    ...
    typed surface in :class:`~sfmapi.server.schemas.api.common.ProblemResponse`.
    """
...
class TenantViolationError(SfmApiError):
    ...
    Raised by :func:`sfmapi.server.core.tenancy.current_tenant` when the
...
class CapabilityUnavailableError(SfmApiError):
    ...
    the canonical feature name (see :mod:`sfmapi.server.core.capabilities`) so
