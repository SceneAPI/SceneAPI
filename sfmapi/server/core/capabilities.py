...
# Together with the operation registry (sfmapi.server.core.operations, which owns the
...
def detect_capabilities() -> Capabilities:
    ...
    Asks the active :class:`~sfmapi.server.adapters.backend.Backend` for its
    ...
    try:
        from sfmapi.server.adapters.backend_actions import has_backend_actions, list_backend_actions
    ...
    try:
        from sfmapi.server.adapters.backend_config import has_backend_config_schemas
    ...
    try:
        from sfmapi.server.adapters.backend_artifacts import has_backend_artifact_contracts
    ...
    if unknown:
        _log.warning(
            ...
            hint=(
                "add these to sfmapi.server.core.capabilities.ALL_KNOWN or remove them "
                "from the backend's capabilities() set"
            ),
        )
    # sfmapi-internal capabilities — independent of the SfM backend.
    from sfmapi.server.core.optional_deps import has_pillow

    if has_pillow():
        caps.features["similarity.dhash"] = True
        caps.features["images.thumbnail"] = True
    try:
        from sfmapi.server.core.projection_engine import has_projection_engine
