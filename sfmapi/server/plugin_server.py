...
server: a plugin packages its :class:`~sfmapi.server.adapters.backend.Backend` + a task
...
class ManifestBackend:
    ...
    in-process :class:`~sfmapi.server.adapters.backend.Backend` object to serve. This
...
def build_plugin_server(
    ...
    *,
    ...
    def _extension_catalog() -> Any:
        ...
        ``sfmapi.server.core.capabilities.ALL_KNOWN`` is misconfigured, and every
        ...
        if unknown:
            raise ValueError(
                ...
                f"vocabulary (sfmapi.server.core.capabilities.ALL_KNOWN): {', '.join(unknown)}"
            )
    ...
    async def actions() -> dict[str, Any]:
        try:
            from sfmapi.server.adapters.backend_actions import list_backend_actions
    ...
    async def validate_action(action_id: str, params: dict[str, Any] = Body(default={})):
        try:
            from sfmapi.server.adapters.backend_actions import validate_backend_action
