...
`sfmapi.server.adapters.*` and are imported only inside worker processes.
"""
...
from sfmapi.server.api.v1 import health
from sfmapi.server.core.config import Settings, get_settings
from sfmapi.server.core.errors import SfmApiError
from sfmapi.server.core.ids import new_id
from sfmapi.server.core.logging import bind_request_context, configure_logging, get_logger
from sfmapi.server.core.profiling import RequestProfilingMiddleware
...
class RequestIdMiddleware(BaseHTTPMiddleware):
    ...
    falls back to a fresh ULID via :func:`sfmapi.server.core.ids.new_id` when
    ...
    :func:`sfmapi.server.core.logging.bind_request_context`.
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    if settings.ephemeral:
        ...
        # branch is unused.
        from sfmapi.server.db import models as _models  # noqa: F401 — register tables
    ...
    try:
        ...
        if janitor_task is not None:
            ...
            with contextlib.suppress(asyncio.CancelledError, Exception):
                await janitor_task
        if settings.ephemeral:
            from sfmapi.server.db.session import _engine as _shared_engine
...
def create_app() -> FastAPI:
    ...
    from sfmapi.server.api.v1 import (
        admin,
        artifacts,
        backend,
        camera_models,
