...
``sfmapi/server/workers/dispatcher.py`` acquires a lease on every task it runs and
...
``sfmapi/server/main.py::lifespan`` (``_janitor_loop``); the DB predicates are kept as
...
from sfmapi.server.db.models import Job, JobEvent, Reconstruction, StageArtifact, Task
from sfmapi.server.orchestrator.lease import now_utc
from sfmapi.server.orchestrator.queue import get_shared_queue
from sfmapi.server.orchestrator.readiness import dependencies_ready, dependency_state
...
async def run_janitor_once(session: AsyncSession) -> list[str]:
    ...
    with contextlib.suppress(Exception):
        from sfmapi.server.services.upload_service import gc_expired_uploads
