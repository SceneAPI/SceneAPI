...
(``sfmapi_client_gen``), which ships in the sibling ``sfmapi-sdk``
...
so ``import sfmapi_client_gen`` resolves from the checkout. Override
...
if (_GEN_PARENT / "sfmapi_client_gen").is_dir() and str(_GEN_PARENT) not in sys.path:
