...
(``scenesdk``), which ships in the sibling ``sfmapi-sdk``
...
so ``import scenesdk`` resolves from the checkout. Override
...
if (_GEN_PARENT / "scenesdk").is_dir() and str(_GEN_PARENT) not in sys.path:
