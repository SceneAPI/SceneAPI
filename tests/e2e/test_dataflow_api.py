...
async def test_installed_plugin_extensions_are_effective(monkeypatch) -> None:
    ...
    async with await _client() as client:
        ...
        assert {attr["name"] for attr in by_id["typed.train"]["special_attributes"]} == {
            "typed.temperature"
        }
        assert by_id["features"]["special_inputs"]["typed.mask"]["datatype"] == "typed.typed_mask"

        pipeline_contract = (await client.get("/v1/pipelines")).json()
        pipelines_by_id = {row["pipeline_id"]: row for row in pipeline_contract["plugin_pipelines"]}
...
async def test_plugin_local_datatype_ids_are_canonicalized_per_plugin(
    ...
    async with await _client() as client:
        ...
        processors = (await client.get("/v1/processors")).json()
        by_id = {row["processor_id"]: row for row in processors["processors"]}
        assert by_id["pluga.producer"]["supplier"]["field"]["datatype"] == ("pluga.local_field")
        assert by_id["plugb.producer"]["supplier"]["field"]["datatype"] == ("plugb.local_field")
        ...
        r = await client.post(
            ...
            json={
                "steps": [
                    ...
                    {"ref": "verify", "processor": "verify", "wires": {"matches": "match.matches"}},
                    {
                        "ref": "map",
                        "processor": "map",
                        "wires": {
...
async def test_pipeline_step_provider_rejects_overlong_selector_component() -> None:
    async with await _client() as client:
        ...
        body = r.json()
        assert any(
            error["type"] == "string_pattern_mismatch" and error["loc"][-1] == "provider"
            for error in body["errors"]
        )
        ...
        r = await client.post(
            ...
            json={
                "steps": [
                    ...
                    {"ref": "verify", "processor": "verify", "wires": {"matches": "match.matches"}},
                    {"ref": "map", "processor": "map", "wires": {"features": "extract.features"}},
                ],
            },
        )
        assert r.status_code == 200, r.text
