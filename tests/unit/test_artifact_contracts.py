...
from sceneapi.server.adapters import backend_artifacts
...
from sceneapi.server.db.models import Job, Project, StageArtifact, Task, utcnow
from sceneapi.server.services import artifact_service
...
def test_file_uri_parser_accepts_windows_drive_file_uri() -> None:
    from sceneapi.server.services.artifact_conversion_service import _file_uri_to_path
...
def test_public_error_message_redacts_urls_paths_and_secrets() -> None:
    ...
    message = (
        ...
        "SCENEAPI_TOKEN=SECRET_BODY and C:/users/me/private.db"
    )
...
def test_job_out_sanitizes_public_error_message() -> None:
    from sceneapi.server.schemas.api.jobs import JobOut
...
async def test_backend_artifact_contract_catalog_is_discoverable(
    ...
    monkeypatch.setenv("SCENEAPI_BACKEND", "artifact_test")
...
async def test_artifact_conversion_supports_multihop_paths(db_setup, monkeypatch) -> None:
    monkeypatch.setenv("SCENEAPI_BACKEND", "artifact_multihop")
    register_backend(
        "artifact_multihop",
...
def test_public_resource_specs_sanitize_backend_options() -> None:
    ...
    from sceneapi.server.schemas.api.radiance import RadianceEvaluationOut, RadianceFieldOut
    from sceneapi.server.schemas.api.reconstructions import ReconstructionOut
...
async def test_radiance_metrics_routes_sanitize_persisted_json(
    ...
    from sceneapi.server.api.v1 import radiance as radiance_routes
