...
from sceneapi.server.scaffolding import (
    ScaffoldedFile,
    _to_class_name,
    scaffold_plugin,
    validate_plugin_id,
...
def test_cli_scaffold_plugin_creates_files(tmp_path: Path) -> None:
    """End-to-end sanity: invoke `python -m sceneapi.server.cli scaffold-plugin <id>`
...
def test_validate_contract_name_accepts_and_rejects() -> None:
    from sceneapi.server.scaffolding import validate_contract_name
...
def test_scaffold_contract_writes_module_and_test(tmp_path: Path) -> None:
    from sceneapi.server.scaffolding import scaffold_contract
