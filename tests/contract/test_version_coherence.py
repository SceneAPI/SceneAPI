...
(``sceneapi.server.__version__``), the wheel metadata (``pyproject.toml``), and
...
from sceneapi.server import __version__ as app_version
...
def test_app_version_matches_pyproject() -> None:
    assert app_version == _pyproject_version(SERVER_ROOT / "pyproject.toml"), (
        "sceneapi.server.__version__ and pyproject.toml [project].version disagree — "
