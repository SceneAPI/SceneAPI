...
``app/mcp/tools.py`` is a hand-maintained mirror of a curated subset of
...
``app.mcp.tools`` / ``app.mcp.server`` module state, which never import
...
from app.mcp.server import TOOL_TITLES
...
def test_every_mcp_tool_maps_to_a_live_rest_route() -> None:
    ...
    Enumerates the registered tools from ``app.mcp.tools.TOOLS`` (no
    ...
    assert not stale, (
        ...
        "remove the entries or restore the tools in app/mcp/tools.py"
    )
...
async def test_portable_stage_catalog_points_at_live_routes() -> None:
    ...
    The catalog in ``app/mcp/tools.py`` hand-copies stage route
    templates ("keep in sync with app/api/v1/..." per its comment).
...
def test_tool_titles_reference_only_registered_tools() -> None:
    ...
    weight that signals a half-finished rename in ``app/mcp/server.py``.
    """
...
async def test_fastmcp_registration_mirrors_module_tool_list() -> None:
    """Invariant: the served MCP surface == ``app.mcp.tools.TOOLS``.
